...
team add mg6
team join mg6 @s[tag=!T0]
execute at @s[tag=PG] if data block -76 9 81 {Text3:'{"obfuscated":true,"color":"gold","clickEvent":{"action":"run_command","value":"tag @s[tag=!T0] add SPmg6"},"text":"@"}'} run tag @s add SPmg6
scoreboard players add @s[tag=!T0] mg6 0
tp @s[scores={mg6=0..}] -67 3 76
tp @s[tag=SPmg6] 100 50 -17
particle cloud -67 4 76 1 1 1 0.1 3000 force @s[tag=P0ft,tag=!N0,scores={mg6=0..}]
tellraw @s[scores={mg6=0..}] ["",{"text":"\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n[","color":"yellow","bold":true},{"text":"@","color":"green","obfuscated":true},{"text":"]","color":"yellow","bold":true},{"text":"---","color":"gold"},{"text":"MINIGAME INFO","color":"aqua","bold":true},{"text":"---","color":"gold"},{"text":"[","color":"yellow","bold":true},{"text":"@","color":"green","obfuscated":true},{"text":"]","color":"yellow","bold":true},{"text":"\n\n\u21b1","color":"gold"},{"text":" Do not drop into sizzling hot lava, the floor shall crumble beneath you!"}]
title @s[tag=!T0] clear
title @s[tag=!T0] reset
title @s[tag=PG] title ["",{"text":"Volcano","color":"gold","bold":true},{"text":" Floor","color":"yellow","bold":true}]
title @s[tag=PG,tag=SPmg6] title ["",{"text":"Spectating","color":"gray","bold":true}]
...
execute unless entity @s[tag=SPmg6] store result score @s c if entity @a[scores={mg6=0..}]
scoreboard players reset @s[tag=!T0,tag=!SPmg6] lobby

execute at @s[tag=T0] unless score @s pg1.1 = @e[limit=1,name="M@K"] pg1 run summon armor_stand -326 2 -13 {Tags:[PG,pre,mg6],Invisible:1,Small:1,Marker:1,NoBasePlate:1}
