...
team add mg5
team join mg5 @s[tag=!T0]
execute at @s[tag=PG] if data block -77 9 81 {Text3:'{"obfuscated":true,"color":"gold","clickEvent":{"action":"run_command","value":"tag @s[tag=!T0] add SPmg5"},"text":"@"}'} run tag @s add SPmg5
scoreboard players set @s[tag=!T0] mg5 0
tp @s[scores={mg5=0..}] -67 3 76
tp @s[tag=SPmg5] -5 8 35
particle cloud -67 4 76 1 1 1 0.1 3000 force @s[tag=P0ft,tag=!N0,scores={mg5=0..}]
tellraw @s[scores={mg5=0..}] ["",{"text":"\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n[","color":"yellow","bold":true},{"text":"@","color":"green","obfuscated":true},{"text":"]","color":"yellow","bold":true},{"text":"---","color":"gold"},{"text":"MINIGAME INFO","color":"aqua","bold":true},{"text":"---","color":"gold"},{"text":"[","color":"yellow","bold":true},{"text":"@","color":"green","obfuscated":true},{"text":"]","color":"yellow","bold":true},{"text":"\n\n\u21b1","color":"gold"},{"text":" Dodge an avalanche of snowballs by taking shelter underneath wooden platforms. Getting caught in it will eliminate you!"},{"text":" \u21b2","color":"gold","bold":true}]
title @s[tag=!T0] clear
title @s[tag=!T0] reset
title @s[tag=PG] title ["",{"text":"Avalanche","color":"aqua","bold":true}]
title @s[tag=PG,tag=SPmg5] title ["",{"text":"Spectating","color":"gray","bold":true}]
...
execute unless entity @s[tag=SPmg5] store result score @s c if entity @a[scores={mg5=0..}]
scoreboard players reset @s[tag=!T0,tag=!SPmg5] lobby

execute at @s[tag=T0] unless score @s pg1.1 = @e[limit=1,name="M@K"] pg1 run summon armor_stand -326 2 -13 {Tags:[PG,pre,mg5],Invisible:1,Small:1,Marker:1,NoBasePlate:1}
