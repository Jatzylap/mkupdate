...
execute as @e[limit=1,name="M@K"] at @s unless data block -325 4 -4 {Text3:'{"bold":true,"color":"yellow","text":"Game decision"}'} unless data block -325 4 -4 {Text3:'{"bold":true,"color":"yellow","text":"Party vote"}'} store result score @s pg1.5 if entity @e[tag=PG,tag=!pre,nbt={Small:1b}]
function clear:chat
