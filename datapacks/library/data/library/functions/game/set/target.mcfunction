...
tag @s add mgc.st0re
execute as @e[limit=1,tag=mgc_m0b_d_t,sort=random,tag=!mgc_st0re] at @s unless score @s mgc-2.1 = @e[tag=mgc.st0re,limit=1] mgc-2.1 positioned as @e[tag=mgc.st0re,limit=1] run tp @s ~ ~1.1 ~
execute unless entity @e[dy=1,tag=mgc_st0re] as @e[tag=mgc_m0b_d_t,sort=nearest,tag=!mgc_st0re,limit=1] at @s run tag @s add mgc_st0re
