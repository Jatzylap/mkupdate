...
execute as @s[tag=0n2] at @s run data merge block -290 15 58 {Text3:'{"bold":true,"color":"red","clickEvent":{"action":"run_command","value":"/function library:game/toggle/button"},"extra":[{"bold":false,"color":"yellow","text":" >>"}],"text":"Nothing"}'}
execute if entity @s[tag=0n2] run playsound ui.button.click master @s ~ ~ ~ 0.2 1
...
execute if entity @s[tag=0n3] run playsound ui.button.click master @s ~ ~ ~ 0.2 0.7
tag @s remove 0n1
tag @s remove 0n2
tag @s remove 0n3