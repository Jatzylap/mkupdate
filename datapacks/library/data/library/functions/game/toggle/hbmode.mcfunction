...
execute unless entity @a[scores={mgc=-1..}] run fill -307 1 71 -291 11 55 air replace barrier
...
execute if score @s c matches 0 if data block -307 15 72 {Text2:'{"bold":true,"color":"aqua","text":"ON"}'} unless entity @a[scores={mgc=-1..}] run tellraw @s ["",{"text":"Build A Floor first!\nOtherwise you'll drop into void.","color":"red"}]
