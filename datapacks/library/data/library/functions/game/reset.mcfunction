...
execute if entity @e[tag=3vent,tag=mgc_p] run scoreboard objectives setdisplay belowName
...
clear @a[scores={mgc=0}]
tag @a[scores={mgc=-1..}] remove mg0
tag @a[scores={mgc=-1..}] remove _rel_
tag @a remove mgc_c
...
execute as @e[tag=mgc_temp] at @s run tp ~ -1000000 ~
execute as @e[tag=mgc_m0b] at @s run tp ~ -1000000 ~
kill @e[tag=3vent_bl0ck]
kill @e[tag=mgc_m0b_d]
kill @e[tag=mgc_m0b_d_t]
kill @e[tag=mgc-score]
kill @e[tag=mgc,type=marker]
kill @e[type=experience_orb,x=-307,y=0,z=55,dx=16,dy=14,dz=16]
...
fill -318 2 69 -332 2 69 air replace repeater[delay=4,facing=east]
...
execute unless blocks -337 2 57 -316 13 69 -299 14 63 masked run setblock -317 2 69 crimson_wall_sign[facing=north]{Text4:'{"text":"\\u26a0","color":"red","bold":true}',Text1:'{"text":"DO NOT PLACE","color":"white","bold":true}',Text2:'{"text":"ANYTHING HERE","bold":true,"color":"white"}',Text3:'{"text":"(","color":"white","extra":[{"text":"redstone signal","color":"green"},{"text":")","color":"white"}]}'} destroy
...
execute unless blocks -356 2 71 -340 13 87 -299 14 63 masked unless entity @a[scores={mgc=-1..}] run schedule function library:game/reset/platform 7
...
execute unless blocks -356 2 71 -340 13 87 -299 14 63 masked if entity @a[scores={mgc=-1..}] run schedule function library:game/reset/platform 87
