team remove mgc
scoreboard players set @a[tag=SPmgc] lobby 1
tag @a remove SPmgc
...
execute as @a[scores={mgc=-1..},tag=!PG] at @s if data block -290 14 59 {Text2:'{"extra":[{"bold":true,"color":"white","text":"Reset:"},{"bold":true,"color":"aqua","text":" MANUAL"}],"text":""}'} run function mk:restore
...
execute unless blocks -356 2 71 -340 23 87 -299 14 63 masked run fill -307 -10 71 -291 11 55 air
execute unless blocks -356 1 88 -335 12 100 -299 14 63 masked run fill -337 2 57 -316 12 69 air
