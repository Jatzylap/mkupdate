...
execute if entity @e[tag=3vent,tag=start,tag=!gm1] run gamemode adventure @a[scores={mgc=-1}]
execute if entity @e[tag=3vent,tag=start,tag=gm1] run gamemode survival @a[scores={mgc=-1}]
...
execute if data block -290 16 59 {Text2:'{"bold":true,"color":"green","text":"Yes"}'} run tag @a[scores={mgc=-1}] add mg0
...
execute if entity @e[tag=entity,tag=3vent] run schedule function library:game/loop/entity 7
